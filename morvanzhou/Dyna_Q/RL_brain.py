...
class QLearningTable:
	...
	def choose_action(self, observation):
      ...
      #print("1:", state_action.dtypes)
			state_action = state_action.reindex(np.random.permutation(state_action.index)) # some actions have same value
			#print("2:", state_action.dtypes)
      state_action = state_action.astype('float64') # very important
      action = state_action.argmax()
			state_action = state_action.astype('float64')
		else:
...
居然把low这些float类型全部转为了object，所以解决办法就是把这些列的数据转为float64.
